<?php
	if ($_SESSION['login']=="yes" and $_SESSION['log_ss'] != ""){
		...
		if ($itemCount > 0){
			foreach ($result as $row){
				//$orgType = $row['organization_type_id'];
				if ($licenseID != "1000" and ($memberID == $row['admin1'] or $memberID == $row['admin2'])){
					$showGroupSetup = "yes";
				}
			}
		...
		//if ($orgType == "1001"){
		//	$dashboard = "m_content/churchview.php";
		//	$sidebar = "m_content/churchbar.php";
		//}else{
			$sidebar = "m_content/sidebar.php";
			$dashboard = "m_content/budgetview.php";
		//}
		...
		if ($showGroupSetup == "yes"){
			$groupSetup = $groupSetup.'<tr><th><a href="'.$mx005gs.'">Group setting</a></th></tr>';
		}
