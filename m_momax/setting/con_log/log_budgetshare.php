<?php
	if ($_SESSION['login']=="yes" and $_SESSION['log_ss'] != ""){
		...
		try{//count budgets				
			$result = $db->prepare("SELECT budget_id FROM $db_budget WHERE member_id=? AND active=?");
			$result->execute(array($memberID,$yes));
		} catch(PDOException $e) {
			print "<script> self.location='".$index_url."?err=d1000'; </script>";
		}
		$budgetCount = $result->rowCount();
			...
			for ($i = 0; $i < $allGrpMembersCt; $i++){
				...
				foreach ($result as $row){
					if ($shareMemberCt == 0 and $row['member_id'] != $memberID){
						...
						$shareMemberCt++;
						$curExistMemberList = $curExistMemberList.'<tr><td><a href="#" onclick="shareAccount('.$row['member_id'].','.$memberID.','.$budgetCount.',2)">'.$memberTotInst->memberFLNameF($row['member_id']).'</td><td>'.$memberTotInst->memberGroupF($allGrpMembers[$i]).'</td><td></td></tr>';
					}else{
						$shareMemIDAdd = "no";
		...
		$itemCount = $result->rowCount();
		if ($itemCount > 0){
			$accountList = $accountList.'<div class="form-group"><label class="col-lg-4 control-label">MY BUDGETS:</label><div class="col-lg-7"><div class="radio"><i>Level1(read only), Level2(edit), & Level3(add/delete)</i></div></div></div>';
			foreach ($result as $row){
				...
				$accountList = $accountList.'<div class="radio"><label><input type="radio" name="'.$row['budget_id'].'" id="a'.$row['budget_id'].'" value="0">No &nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['budget_id'].'" id="b'.$row['budget_id'].'" value="1">Level1&nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['budget_id'].'" id="c'.$row['budget_id'].'" value="2">Level2&nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['budget_id'].'" id="d'.$row['budget_id'].'" value="3">Level3</label></div></div></div>';
			}
		}else{
			$accountList = "No budgerts available";
		}
