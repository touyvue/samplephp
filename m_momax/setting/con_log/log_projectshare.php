<?php
	if ($_SESSION['login']=="yes" and $_SESSION['log_ss'] != ""){
		...
		try{//count accounts				
			$result = $db->prepare("SELECT project_id FROM $db_project WHERE member_id=? AND active=?");
			...
			for ($i = 0; $i < $allGrpMembersCt; $i++){
				...
				foreach ($result as $row){
					if ($shareMemberCt == 0 and $row['member_id'] != $memberID){
						...
						$shareMemberCt++;
						$curExistMemberList = $curExistMemberList.'<tr><td><a href="#" onclick="shareAccount('.$row['member_id'].','.$memberID.','.$projectCount.',3)">'.$memberTotInst->memberFLNameF($row['member_id']).'</td><td>'.$memberTotInst->memberGroupF($allGrpMembers[$i]).'</td><td></td></tr>';
					}else{
						$shareMemIDAdd = "no";
		...
		$itemCount = $result->rowCount();
		if ($itemCount > 0){
			$accountList = $accountList.'<div class="form-group"><label class="col-lg-4 control-label">MY EVENTS:</label><div class="col-lg-7"><div class="radio"><i>Level1(read only), Level2(edit), & Level3(add/delete)</i></div></div></div>';
			foreach ($result as $row){
				...
				$accountList = $accountList.'<div class="radio"><label><input type="radio" name="'.$row['project_id'].'" id="a'.$row['project_id'].'" value="0">No &nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['project_id'].'" id="b'.$row['project_id'].'" value="1">Level1&nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['project_id'].'" id="c'.$row['project_id'].'" value="2">Level2&nbsp;</label>';
				$accountList = $accountList.'<label><input type="radio" name="'.$row['project_id'].'" id="d'.$row['project_id'].'" value="3">Level3</label></div></div></div>';
			}
		}else{
			$accountList = "No events available";
		}
